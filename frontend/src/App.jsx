...
const s = {
  ...
  srcBadge: (src) => ({
    ...
    background: src === "wiktionary" ? "#e8f4e8" : src === "claude" ? "#e8eef8" : "#f0f0f0",
    color: src === "wiktionary" ? "#2d6a0f" : src === "claude" ? "#1a3a7a" : "#666",
  }),
};
...
export default function App() {
  const [page, setPage] = useState(0); // 0=sceltaFrase 1=lookup 2=userInput 3=trascrizione
  ...
  const [infoParole, setInfoParole] = useState([]); // from /api/lookup
  const [needsInput, setNeedsInput] = useState([]); // words needing user input
  const [userChoices, setUserChoices] = useState({}); // {word: {v?, sz?}}
  ...
  function allChoicesDone() {
    return needsInput.every(word => {
      ...
      // We need to know what's missing for this word — simplify: always ask both
  ...
  function reset() {
    ...
    setGenMsg(""); setInfoParole([]); setNeedsInput([]); setUserChoices({});
    setIpaValue(""); setFeedback(null);
  }

  function buildNoteText() {
    if (!infoParole.length && !Object.keys(userChoices).length) return "Nessuna informazione dizionario";
    const parts = [];
    infoParole.forEach(info => {
      ...
      if (pp.length) parts.push(`${w}[${src}]: ${pp.join(", ")}`);
    });
    Object.entries(userChoices).forEach(([w, ch]) => {
      const pp = [];
      if (ch.v) pp.push(`vocale→${ch.v}`);
      if (ch.sz) pp.push(`sz→${ch.sz}`);
      if (pp.length) parts.push(`${w}[utente]: ${pp.join(", ")}`);
    });
    return parts.length ? parts.join(" · ") : "Regime convenzionale";
  }
        ...
        <>
          ...
          <div style={s.card}>
            ...
            <p style={{ fontFamily: "system-ui", fontSize: 13, color: "#888", marginBottom: 14 }}>
              Per le seguenti parole non è stato possibile trovare informazioni nel dizionario né tramite l'oracolo. Specifica tu:
            </p>
            {needsInput.map(word => {
              const ch = userChoices[word] || {};
              return (
                <div key={word} style={{ padding: "12px 0", borderBottom: "1px solid #f0f0f0" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      <button onClick={() => setChoice(word, "v", "e")} style={ch.v === "e" ? cbtnSel : cbtn}>e chiusa</button>
                      <button onClick={() => setChoice(word, "v", "ɛ")} style={ch.v === "ɛ" ? cbtnSel : cbtn}>ɛ aperta</button>
                      <button onClick={() => setChoice(word, "v", "o")} style={ch.v === "o" ? cbtnSel : cbtn}>o chiusa</button>
                      <button onClick={() => setChoice(word, "v", "ɔ")} style={ch.v === "ɔ" ? cbtnSel : cbtn}>ɔ aperta</button>
                      <button onClick={() => setChoice(word, "v", "n/a")} style={ch.v === "n/a" ? cbtnSel : cbtn}>n/a</button>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <span style={{ fontSize: 12, color: "#999", fontFamily: "system-ui", minWidth: 110 }}>s/z intervocalica</span>
                      <button onClick={() => setChoice(word, "sz", "s")} style={ch.sz === "s" ? cbtnSel : cbtn}>s sorda</button>
                      <button onClick={() => setChoice(word, "sz", "z")} style={ch.sz === "z" ? cbtnSel : cbtn}>z sonora</button>
                      <button onClick={() => setChoice(word, "sz", "ts")} style={ch.sz === "ts" ? cbtnSel : cbtn}>ts sorda</button>
                      <button onClick={() => setChoice(word, "sz", "dz")} style={ch.sz === "dz" ? cbtnSel : cbtn}>dz sonora</button>
                      <button onClick={() => setChoice(word, "sz", "n/a")} style={ch.sz === "n/a" ? cbtnSel : cbtn}>n/a</button>
                    </div>
                  </div>
                </div>
              );
          ...
          <div style={s.card}>
            ...
            <div style={s.note}>
              {infoParole.map(info => {
                ...
                if (info.has_open_o) pp.push("o→ɔ");
                if (info.has_voiced_s) pp.push("s→z");
                if (info.has_voiced_z || info.has_voiceless_z) pp.push("z→" + (info.has_voiced_z ? "dz" : "ts"));
                return pp.length ? (
                  <span key={info.word} style={{ marginRight: 12 }}>
                    <em>{info.word}</em>
                    <span style={s.srcBadge(info.source)}>{info.source}</span>
                    {": " + pp.join(", ")}
                  </span>
                ) : null;
              })}
              {Object.entries(userChoices).map(([w, ch]) => {
                const pp = [];
                if (ch.v && ch.v !== "n/a") pp.push(`vocale→${ch.v}`);
                if (ch.sz && ch.sz !== "n/a") pp.push(`sz→${ch.sz}`);
                return pp.length ? (
                  <span key={w} style={{ marginRight: 12 }}>
                    <em>{w}</em>
                    <span style={s.srcBadge("utente")}>utente</span>
                    {": " + pp.join(", ")}
                  </span>
                ) : null;
              })}
